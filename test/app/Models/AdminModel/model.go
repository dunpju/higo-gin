...
func (this *Impl) GetByAdminId(AdminId int64, columns ...string) *gorm.DB {
	return this.Mapper(squirrel.Select(columns...).From(this.TableName()).Where("`admin_id` = ?", AdminId).ToSql()).Query()
}

func (this *Impl) GetByAdminIds(AdminIds []string, columns ...string) *gorm.DB {
	return this.Mapper(squirrel.Select(columns...).From(this.TableName()).Where("`admin_id` IN(?)", strings.Join(AdminIds, ",")).ToSql()).Query()
}
